...
static const std::string BASE_ROOT = "/root/webserver/web/resource";

void onRequest(const HttpRequest& req, HttpResponse* resp)
{
    LOG_INFO << "Headers " << req.getMethodStr() << " " << req.getPath() ;
    if (!benchmark)
    {
        const std::unordered_map<std::string, std::string>& headers = req.getHeader().getHeaders();
    ...
    {
        resp->setStatusCode(HttpStatusCode::OK);
        resp->setContentType(HttpContentType::PNG);
        resp->setBody(std::string(favicon, sizeof favicon));
    }
    else if (req.getPath()  == "/hello")
    {
        resp->setStatusCode(HttpStatusCode::OK);
        resp->setContentType(HttpContentType::PLAIN);

        resp->setBody("hello, world!\n");
    }
    else if (req.getPath()  == "/json")
    {
        resp->setHttpResponseLine(req.getVersion(), HttpStatusCode::OK);
        resp->setContentType(HttpContentType::JSON);
        nlohmann::json j ;
        j["date"] = TimeStamp::now().toFormatString();
        j["messsage"] = "hello world!";
        resp->setJsonBody(j);
    }
    else if (req.getPath()  == "/dog")
    {
        ...
        if(exists){
            ...
            if(accpetable_exts.find(ext) != accpetable_exts.end()){
                ...
                resp->setContentType(HttpContentType::HTML);
                resp->appendBody("I don't know what do you really want");
            }
        }
        else{
...
int main(int argc, char* argv[])
{
    ...
    server.addServlet("/json", [](HttpRequest request, HttpResponse* response) {
        onRequest(request, response);
    });
    server.addServlet("/hello", [](HttpRequest request, HttpResponse* response) {
        onRequest(request, response);
    });
    server.addServlet("/favicon.ico", [](HttpRequest request, HttpResponse* response) {
        onRequest(request, response);
    });
    server.addServlet("/dog", [](HttpRequest request, HttpResponse* response) {
        onRequest(request, response);
    });
