...
namespace Tiny_muduo
{
    namespace log
    {
        AsyncLog::AsyncLog(const std::string& basename, int flush_interval, off_t roll_size)
                : _flush_interval(flush_interval),
                 _roll_size(roll_size), _running(true), _thread(std::bind(&AsyncLog::writeThread, this), "AsyncLogThread"),
                  _basename(basename), _cur(new Buffer), _next(new Buffer), _buffers()
        {
            _cur->clear();
            _next->clear();
            _buffers.reserve(16);
