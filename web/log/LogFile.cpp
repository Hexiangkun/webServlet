...
namespace HLog
{
    ...
    LogFile::LogFile(const std::string& filepath, const std::string& basename, off_t roll_size)
            : _filepath(filepath),
            ...
            _baseName(basename)
    {
        if(!(std::filesystem::exists(filepath) && std::filesystem::is_directory(filepath))) {
            if(!std::filesystem::create_directories(filepath)) {
                throw std::logic_error("create directories failed!");
            }
        }
        rollFile();
    }
    ...
    void LogFile::rollFile() {
        ...
        _file.reset(new FileWriter<FileBufSize>(_filepath+ "/"+filename));
    }
